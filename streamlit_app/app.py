...
st.markdown('<div class="subtitle">Enterprise candidate match evaluator & qualitative rankings</div>', unsafe_allow_html=True)
...
tab1, tab2, tab3 = st.tabs(["📋 Upload & Settings", "📊 Match Analysis", "📂 Export & Reports"])
...
with tab1:
    ...
    if st.button("Start AI Screening Workflow", type="primary"):
        if not jd_file:
            ...
            st.info("Screening process triggered. Graph workflow skeleton will execute when business logic is implemented.")
...
with tab3:
    ...
    st.write("Extract analysis logs, candidate list indices, and summaries in format of choice.")
...
st.divider()
st.caption(f"System Running Framework: FastAPI Backend / Streamlit UI • Version 1.0.0 • Local Time: {datetime.now().strftime('%Y-%m-%d %H:%M')}")
